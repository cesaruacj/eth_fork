...
const ETHERSCAN_API: string = 'https://api.etherscan.io/api';

// DEX contracts - mantener igual
const DEX_CONTRACTS: DexContracts = {
  UniswapV2: {
...
const ABI_DIR: string = path.join(__dirname, 'external', 'abis', network);
if (!fs.existsSync(ABI_DIR)) {
  fs.mkdirSync(ABI_DIR, { recursive: true });
}

// Función para descargar ABI
async function downloadABI(contractAddress: string, dexName: string, contractType: string): Promise<boolean> {
  try {
    process.stdout.write(`Descargando ABI para ${dexName} ${contractType} (${contractAddress})... `);
    
    const url: string = `${ETHERSCAN_API}?module=contract&action=getabi&address=${contractAddress}&apikey=${API_KEY}`;
    const response = await axios.get(url);
    
    if (response.data.status === '1') {
      // Crear directorio para el DEX si no existe
      const dexDir: string = path.join(ABI_DIR, dexName);
      if (!fs.existsSync(dexDir)) {
        fs.mkdirSync(dexDir, { recursive: true });
      }
      
      // Guardar ABI
      const abiPath: string = path.join(dexDir, `${contractType.toLowerCase()}.json`);
...
async function downloadAllABIs(): Promise<void> {
  ...
  for (const [dexName, contracts] of Object.entries(DEX_CONTRACTS)) {
    for (const [contractType, address] of Object.entries(contracts)) {
      const success: boolean = await downloadABI(address, dexName, contractType);
...
// Función para descargar ABI con reintentos
async function downloadABIWithRetry(contractAddress: string, dexName: string, contractType: string, retries = 2): Promise<boolean> {
  try {
    return await downloadABI(contractAddress, dexName, contractType);
  } catch (error) {
    if (retries > 0) {
      console.log(`Retrying after 2 seconds...`);
      await delay(2000);
      return downloadABIWithRetry(contractAddress, dexName, contractType, retries - 1);
    }
    return false;
  }
}
